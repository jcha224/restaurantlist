...
public class Restaurant {
	...
	public void updateRegisters() {
		...
		while(itr.hasNext()) {
			...
			if(temp.checkFinish() && temp.getPresent()) {
	...
	public void updateCustomers() {
		if(myWaitLine.getSize() > 0) {
			...
			while(itr.hasNext()){
				Register temp = itr.next();
				if(!temp.getPresent() && temp.getStatus()) {
	...
	public void writeStats() {
		...
		try {
			...
			writer.println("Number of customers still waiting at the register: " + waitingRegister());
	...
	/**
	 * Returns the number of customers waiting at the registers.
	 * @return customers waiting at the register.
	 */
	public int waitingRegister() {
		int count = 0;
		Iterator<Register> itr = myRegisters.iterator();
		while (itr.hasNext()) {
			Register temp = itr.next();
			if(temp.getStatus() && temp.getPresent()) {
				count++;
			}
		}
		return count;
	...
	/**
	 * Returns the average wait time for the customers who got food.
	...
	public float averageTime() {
		...
		if(totalTime > 0) {
			totalTime = totalTime/myCustomers.size() ;
		}
		return totalTime;
	}
